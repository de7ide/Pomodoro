...
class TaskRepo:
    ...
    async def get_task(self, id: int) -> Tasks | None:
        async with self.db_session as session:
            task: Tasks = (await session.execute(select(Tasks).where(Tasks.id == id))).scalar_one_or_none()
        return task
    ...
    async def update_task_name(self, task_id: int, name: str) -> str:
        ...
        async with self.db_session as session:
            ...
            await session.commit()
            await session.flush()
            return await self.get_task(id=task_id)
    ...
    async def get_task_by_category_id(self, category_id: int) -> list[Tasks]:
